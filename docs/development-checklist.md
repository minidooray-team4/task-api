...
- [X] 요청자의 프로젝트 접근 권한을 검증한다.
- [X] 존재하지 않는 리소스는 `404 Not Found` 계열 비즈니스 예외로 처리한다.
- [X] 권한이 없는 요청은 `403 Forbidden` 계열 비즈니스 예외로 처리한다.
- [X] 생성/수정 요청 DTO에 Bean Validation을 적용한다.
- [X] 서비스 메서드에 트랜잭션 경계를 설정한다.
